...
class TaichuVLRWrapper(BaseAPI):
    ...
    def generate_inner(self, inputs, **kwargs) -> tuple:
        ...
        # 1. Request
        chat_response = requests.post(self.api_url, json=data, headers=headers, stream=stream)
...
class TaichuVLRAPI(TaichuVLRWrapper):
    ...
    def generate(self, message, dataset=None,**kwargs):
        return super(TaichuVLRAPI, self).generate(message, dataset=dataset,**kwargs)
