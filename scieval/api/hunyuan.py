...
class HunyuanWrapper(BaseAPI):
    ...
    def generate_inner(self, inputs, **kwargs) -> tuple:
        ...
        # stream = kwargs.pop('stream', False) # Hunyuan SDK specific handling required for stream
        ...
        payload = dict(
            ...
            TopK=1,
            # Stream=stream, # Uncomment if SDK supports it in payload
            **kwargs)
        ...
        # 3. Call SDK (Non-stream default logic preserved as specific stream method unknown)
        ...
        return 0, answer, resp_json


class HunyuanVision(HunyuanWrapper):

    def generate(self, message, dataset=None,**kwargs):
        return super(HunyuanVision, self).generate(message,**kwargs)
