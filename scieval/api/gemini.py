...
class GeminiWrapper(BaseAPI):
    ...
    def generate_inner(self, inputs, **kwargs) -> tuple:
        ...
        if self.backend == 'genai':
            ...
            if self.media_resolution:
                if video_in_msg:
                    assert self.media_resolution != 'high', "Video input supports medium/low only"
            ...
            if stream:
                # Stream Logic
                resp_iterator = self.client.models.generate_content_stream(
                    model=model,
                    contents=messages,
            ...
            if stream:
                # Stream Logic
                resp_iterator = model.generate_content(messages, stream=True)
                answer = ""
                for chunk in resp_iterator:
                    answer += chunk.text
            else:
...
class Gemini(GeminiWrapper):
    ...
    def generate(self, message, dataset=None,**kwargs):
        return super(Gemini, self).generate(message,**kwargs)
