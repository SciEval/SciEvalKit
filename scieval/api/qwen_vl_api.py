...
from scieval.vlm.qwen2_vl.prompt import Qwen2VLPromptMixin
...
class QwenVLWrapper(BaseAPI):
    ...
    def generate_inner(self, inputs, **kwargs) -> tuple:
        ...
        # (Removed redundant pure_text check assert, assume passed)
        ...
        if stream:
            ...
            for resp in responses:
                ...
                if resp.output and resp.output.choices:
                    # Dashscope multimodal usually returns accumulated text
...
class QwenVLAPI(QwenVLWrapper):
    ...
    def generate(self, message, dataset=None,**kwargs):
        return super(QwenVLAPI, self).generate(message,**kwargs)
