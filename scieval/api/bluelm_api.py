...
class BlueLMWrapper(BaseAPI):
    ...
    def generate_inner(self, inputs, **kwargs) -> tuple:
        ...
        # 2. Prepare prompt and images
        prompt, image_path = self.message_to_promptimg(inputs, kwargs['dataset'])

        # 3. Call external inference function (No stream support in this wrapper)
        response = multimodal(
            images=image_path, text=prompt, url=self.url, key=self.key,
            temperature=self.temperature, max_tokens=self.max_tokens,
            top_k=self.top_k, top_p=self.top_p, timeout=self.timeout
        )

        # 4. Post-process answer (Original Logic)
        dataset_name = kwargs.get('dataset', '')
        raw_answer = response[0]

        answer = split_think(raw_answer)
...
class BlueLM_API(BlueLMWrapper):
    ...
    def generate(self, message, dataset=None,**kwargs):
        return super(BlueLM_API, self).generate(message, dataset=dataset,**kwargs)
