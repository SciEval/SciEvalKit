...
class JTVLChatWrapper(BaseAPI):
    ...
    def get_send_data(self, prompt, image_path, temperature, max_tokens):
        ...
        # }
        send_data = {"prompt": prompt, "image_base64": image}
        return send_data
    ...
    def generate_inner(self, inputs, **kwargs) -> tuple:
        ...
        # Inject stream into payload if needed
        if stream: send_data['stream'] = True
        ...
        # 5. Parse
        if stream:
            # Assumed OpenAI-compatible stream format
            full_content = ""
            ...
            # Original Non-Stream Logic (Preserved `r.text` behavior)
            output = r.text
...
class JTVLChatAPI_Mini(JTVLChatWrapper):
    ...
    def generate(self, message, dataset=None,**kwargs):
        return super(JTVLChatAPI_Mini, self).generate(message, dataset=dataset,**kwargs)
