...
class TeleMMAPI(SiliconFlowAPI):
    ...
    def generate_inner(self, inputs, **kwargs) -> tuple:
        ...
        default_kwargs.update(kwargs)
        stream = default_kwargs.pop('stream', False)
        ...
        payload = dict(model=self.model, messages=messages, stream=stream, **default_kwargs)

        # 1. Request
        response = requests.post(
            self.api_base, headers=self.headers, data=json.dumps(payload), stream=stream
        )
