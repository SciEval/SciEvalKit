...
class JTVLChatWrapper(BaseAPI):
    ...
    def generate_inner(self, inputs, **kwargs) -> tuple:
        ...
        stream = kwargs.pop('stream', False)  # Use framework stream arg, default False

        # 2. Get Send Data
        # Note: Passed stream param to helper functions
        if image_path:
            send_data = self.get_send_data(
                prompt=prompt, image_path=image_path, temperature=self.temperature,
        ...
        # 3. Request
        r = requests.post(
            self.api_base, headers=header_dict, data=json_data,
            timeout=3000, stream=stream
        )

        # 4. Check Error (Propagates to BaseAPI)
        r.raise_for_status()
                    ...
                    try:
                        ...
                        if 'choices' in chunk:
                            for choice in chunk['choices']:
                                ...
                                # print(content, end='', flush=True) # Optional: Print to stdout
...
class JTVLChatAPI(JTVLChatWrapper):
    ...
    def generate(self, message, dataset=None,**kwargs):
        return super(JTVLChatAPI, self).generate(message, dataset=dataset,**kwargs)
