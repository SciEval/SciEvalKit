...
class LMDeployWrapper(BaseAPI):
    ...
    def generate_inner(self, inputs, **kwargs) -> tuple:
        ...
        if dataset and listinstr(['BMMR'], dataset):
            max_tokens = 8196
...
class LMDeployAPI(LMDeployWrapper):
    ...
    def generate(self, message, dataset=None,**kwargs):
        return super(LMDeployAPI, self).generate(message, dataset=dataset,**kwargs)
